    ...
    tools: [
      {
        ...
        inputSchema: zodToJsonSchema(GetRoutineArgsSchema) as ToolInput,
      },
      {
        name: "list_routines",
        description: "List all available dance routines. Can be filtered by style and difficulty level.",
        inputSchema: zodToJsonSchema(ListRoutinesArgsSchema) as ToolInput,
      }
    ],
  };
});
    ...
    switch (name) {
      ...
      case "list_routines": {
        ...
        const filteredRoutines = routines.filter(routine => {
          if (parsed.data.style && routine.style !== parsed.data.style) return false;
          if (parsed.data.difficulty && routine.difficulty !== parsed.data.difficulty) return false;
          return true;
        });
